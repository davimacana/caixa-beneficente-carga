...
def ler_csv_inserir_tabela_dependente(input_file):
    try:
        ...
        with open(input_file, 'r', newline='', encoding='utf-8') as arquivo_csv:
            try:
                ...
                for linha in csv_reader:
                    insert_query = """INSERT INTO public.dependente (cpf, data_adesao, data_cancelamento, data_nascimento, estado_civil, falecido, sexo, nome, conjuge, obs, 
                    orgao_exp, rg, tel_principal, status, id_titular)
                    SELECT %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s
                    WHERE NOT EXISTS (SELECT 1 FROM public.dependente WHERE nome = %s) AND EXISTS (SELECT 1 FROM public.titular WHERE id = %s)
                    """
                    ...
                    dataNascimento = formatar_date('DT_NASCIMENTO', linha['DT_NASCIMENTO'])  # date
                    sexo = formatar_sexo('SEXO', linha['SEXO'])
                    estado_civil = formatar_estado_civil('ESTADO CIVIL', linha['ESTADO_CIVIL'])

                    cursor.execute(insert_query, (
                        linha['CPF'], dataAdesao, dataCancelamento, dataNascimento, estado_civil, "false", sexo, linha['NM_DEPENDENTE'], linha['NM_CONJUGE'],
                        linha['OBS'], linha['ORGAO_EXP'], linha['RG'], linha['TEL_PRINCIPAL'], "true", linha['ID_ASSOCIADO'], linha['NM_DEPENDENTE'], linha['ID_ASSOCIADO']
                    ))
                    conn.commit()
...
if __name__ == "__main__":
    input_file = 'dependente.csv'
    ler_csv_inserir_tabela_dependente(input_file)
    
    #input_file = 'dependente.csv'  
    #output_file = 'dependente2.csv'
    ...
    #input_file = 'dependente3.csv'
    #output_file = 'dependente4_sem_duplicatas.csv'
